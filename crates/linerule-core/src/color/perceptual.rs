...
mod tests {
    ...
    /// Pins both `l_star` segments: below the toe (linear) and above (cube-root).
    #[test]
    fn l_star_segment_values_are_pinned() {
        // Below toe: linear * 9.032962 / 1.16
        // 0.005 → 0.005 * 9.032962 / 1.16 ≈ 0.038935
        let v_toe = l_star(0.005);
        assert!(
            (v_toe - 0.038_935).abs() < 0.001,
            "l_star(0.005) toe segment: expected ≈ 0.03894, got {v_toe}"
        );
        // Above toe: 1.16 * linear.cbrt() - 0.16
        // 0.5 → 1.16 * 0.5^(1/3) - 0.16 ≈ 0.7607
